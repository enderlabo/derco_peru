<template>
  <div class="footer-component">
    <div>
      ...
      <footer>
        <div class="body-footer">
          <b-container class="text-md-left container-footer">
            <section-desktop />
            <section-mobile />
          </b-container>
        </div>
    ...
    <div class="elevated">
      <div class="w-100">
        <div id="footer--cookies" style="position: fixed; bottom: 0">
          <b-alert
            :show="showCookiePanel"
            variant="light"
            class="alert text-center mb-0"
            block
          >
            Al navegar en este sitio aceptas las cookies que utilizamos para
            mejorar tu experiencia. &nbsp;
            <br v-if="$device.isMobile" />
            <b-link href="/privacidad/#cookies" target="_blank">
              <u> Más información </u>
            </b-link>
            &nbsp;
            <b-button size="sm" variant="primary" pill @click="acceptCookie">
              Entendido
            </b-button>
          </b-alert>
        </div>
...
import SectionMobile from '@/components/core/Footer/SectionMobile'
import SectionDesktop from '@/components/core/Footer/SectionDesktop'

export default {
  name: 'Footer',
  components: { SectionDesktop, SectionMobile },
  data() {
    return {
      mainProps: {
        blank: true,
  ...
  methods: {
    ...
    acceptCookie() {
      ...
      localStorage.setItem(
        'acceptCookie',
        JSON.stringify({ accept: true, expirate: this.formatDate() })
      )
    },
    validateCookie() {
      if (localStorage.getItem('acceptCookie')) {
        const data = JSON.parse(localStorage.getItem('acceptCookie'))
        const expirate = new Date(data.expirate)
        const hoy = new Date()
        if (hoy > expirate) {
    ...
    handleScroll() {
      const element = document.getElementById('footer--cookies')
      if (element) {
        const scrollBottom =
